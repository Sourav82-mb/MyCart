...
def mobile(request,data=None):
    if data == None:
        mobiles=Product.objects.filter(category='M')
    elif data == 'Redmi' or data == 'NOKIA' or data == 'Samsung':
...
def laptop(request,data=None):
    if data == None:
        laptops=Product.objects.filter(category='L')
    # elif data == 'below':
        # laptops=Product.objects.filter(category='L').filter(discounted_price_lt=40000)
    # elif data == 'above':
       # laptops=Product.objects.filter(category='L').filter(discounted_price_gt=40000)
    return render(request, 'app/laptop.html',{'laptops':laptops})

def topwear(request,data=None):
    if data == None:
        topwears=Product.objects.filter(category='TW')
    elif data == 'LouisPhillipe' or data == 'Sparky' or data == 'Jenci':
